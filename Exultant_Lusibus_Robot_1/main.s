; Projet Exultant Lusibus Robot 1 by Raphael CANIN et Yann SOBGUI

		AREA    |.text|, CODE, READONLY
 
; This register controls the clock gating logic in normal Run mode
SYSCTL_PERIPH_GPIO EQU		0x400FE108	; pointeur registre de gestion de la clock
...
GPIO_O_DIR   		EQU 	0x00000400  ; configuration des leds en sorties
...
GPIO_O_DEN  		EQU 	0x0000051C  ; active la possibilité de conversion numérique en analogique
...
__main	
		...
		str r2,[r5] ; Allume la led gauche
...
Bumpers_attente; Si l'un des bumpers sont activés, on lance la danse maintenant synchronisée
		...
		str r0,[r5] ; on éteind la led de gauche
...
move7
		...
		BL PARTIE2 ; On demarre la seconde partie de la danse a l'aide du deuxieme fichier .s
...
FIN
		...
		b Switch_attente ; On retourne au début à la fin de la danse
...
WAIT_MESURE	ldr r1, =0xA4F000 ; Duree correspondant à 1 mesure en 4/4 à 118BPM
wait3	subs r1, #1
        bne wait3
		BX	LR
		
WAIT_BLANCHE ldr r1, =0x527800 ; Duree correspondant à 1 blanche à 118BPM
wait8	subs r1, #1
        bne wait8
		BX	LR
...
WAIT_CROCHE	ldr r1, =0x149E00 ; Duree correspondant à 1 croche à 118BPM
wait5	subs r1, #1
        bne wait5
		BX	LR
		
WAIT_DCROCHE	ldr r1, =0xA4F00 ; Duree correspondant à 1 duoble-croche à 118BPM
wait6	subs r1, #1
        bne wait6
		BX	LR	
		
WAIT_TCROCHE	ldr r1, =0x52780 ; Duree correspondant à 1 triple-croche à 118BPM
wait9	subs r1, #1
        bne wait9
		BX	LR		

WAIT_TRIOLET	ldr r1, =0xDBEAB ; Duree correspondant à 1 triolet à 118BPM
wait7	subs r1, #1
        bne wait7
		BX	LR	
